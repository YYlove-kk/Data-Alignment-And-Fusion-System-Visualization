    ...
    <div class="container">
      ...
      <div class="main-content">
        ...
        <div class="align-visual" v-loading="loading">
          ...
          <div ref="heatmapRef" class="heatmap-container" id="chart"></div>
        </div>

        <!-- 对齐详情表 -->
        <el-table :data="alignmentResults" style="width: 100%" border>
...
import { submitPatientDataAnalysis } from "@/api/dataAlignment";
...
const startAlignment = async () => {
  ...
  try {
    // 模拟对齐过程
    loading.value = true;
    // const res = await submitPatientDataAnalysis(alignmentConfig.value)
    // if(res.code === 200){
    //   ElMessage.success("对齐成功");
    //       // 更新指标
    //   metrics.value = res.data.metrics;
    //   alignmentResults.value = res.data.alignmentResults;
    ...
    const res = {
      PatientIds:
        "pa cs01CT201702170412, pacs01CT201702170413, pacs01CT201702170414, pacs01CT201702170415",
      coverage: "6",
      accuracy: 0.75,
      alignmentMatrix:
        "[[0.95, 0.32, 0.78, 0.45], [0.32, 0.89, 0.21, 0.61], [0.78, 0.21, 0.92, 0.80], [0.45, 0.61, 0.80, 0.96]]",
      tableData: [
        {
          textId: "20180101080000_0_txt.npy",
          imageId: "20180101080000_img.npy",
          timeAlignment: "时间差2h",
          similarity: 0.95,
        },
        {
          textId: "20190510093000_1_txt.npy",
          imageId: "20190510093000_img.npy",
          timeAlignment: "时间差2h",
          similarity: 0.89,
        },
        {
          textId: "20200220100000_2_txt.npy",
          imageId: "20170813141500_img.npy",
          timeAlignment: "时间差2h",
          similarity: 0.92,
        },
        {
          textId: "20210615141500_3_txt.npy",
          imageId: "20220303164500_img.npy",
          timeAlignment: "时间差2h",
          similarity: 0.96,
        },
      ],
    ...
    metrics.value = {
      timeError: "2h",
      semanticAccuracy: res.accuracy,
      alignmentCount: res.coverage,
    };

    // 更新结果
    alignmentResults.value = res.tableData;
    const edge = res.PatientIds.split(",");
    const data = res.alignmentMatrix
      .replace(/[\[\]]/g, "") // 移除方括號
      .split(",")
      .map(Number); // 轉換為數字數組
    ...
    for (let i = 0; i < dimension; i++) {
      matrix.push(data.slice(i * dimension, (i + 1) * dimension));
    }

    initHeatmap(edge, matrix);
  } catch (error) {
...
const initHeatmap = (edge, chartData) => {
  ...
  const data = chartData.flatMap((row, i) =>
